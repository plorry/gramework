...
_.extend(Element.prototype, Entity.prototype, {
    initialize: function(options) {
        ...
        if (options.borderImage) {
            this.borderImage = new BorderImage({
                ...
                x: this.rect.left - Math.round((this.borderWidth / 2)),
                y: this.rect.top - Math.round((this.borderWidth / 2)),
    ...
    setPosition: function(x, y) {
        this.rect.left = x - (this.rect.width / 2);
        this.rect.top = y - (this.rect.height / 2);
        if (this.borderImage){
            this.borderImage.rect.left = x - (this.borderImage.rect.width / 2);
            this.borderImage.rect.top = y - (this.borderImage.rect.height / 2);
        }
    }
});
...
var GradientSurface = function(surface, options) {
    ...
    gradSurface.fill("#000");

    var colors = options.colors || [[255, 255, 255]];
    var steps = options.steps || 1;

    var segHeight = gradSurface._canvas.height / steps;
    ...
    colors.forEach(function(color, i) {
        ...
        for (i=0; i < subSteps; i++) {
            var subRect = new gamejs.Rect(
                ...
                [gradSurface._canvas.width, Math.ceil(segHeight)]);
            var thisColor = [
...
var TextBlock = Element.extend({
    initialize: function(options) {
        ...
        this.lineNum = options.lineNum || 3;
    ...
    initText: function() {
        ...
        this.words.forEach(function(word) {
            ...
            if (testWidth > this.rect.width - (2 * this.borderWidth)) {
        ...
        if (!this.rolling) {
            this.lines.forEach(function(line) {
                this.lineSurfaces.push(
                    this.font.render(line, "#000")
                );
            }, this);
        }
    },
    ...
    update: function(dt) {
        ...
        TextBlock.super_.prototype.update.apply(this, arguments);
    },
    ...
    draw: function(surface) {
        ...
        this.lineSurfaces.forEach(function(lineSurface, index, array){
            this.surface.blit(lineSurface, [Math.floor(this.borderWidth), Math.floor((this.lineHeight * index) - this.slide + 2)]);
        }, this);
    },
