...
Scene.prototype.init = function(options) {
    ...
    this.camera = new Camera(this.view.rect, {width:this.width, height:this.height});
};
...
Scene.prototype.draw = function(display) {
    ...
    var screen = this.camera.draw(this.view);
    //var size = screen.getSize();
    //var scaledScreen = gamejs.transform.scale(screen, [size[0] * config.SCALE, size[1] * config.SCALE]);
    gamejs.draw.rect(this.view, 'black', this.camera.rect, 1)
    display.blit(screen);
    //display.blit(this.view, [0,0], new gamejs.Rect([0,0],[1600,1200]));
    //console.log(display);
    //debugger
};
